...
# More specific than the NUFSHT-only `GPUBackend` stub, so it wins when KernelAbstractions is loaded.
# =============================================================================

function FFS._calculate_spectrum_nufsht(exec::FFS.GPUBackend{<:KA.Backend}, g::FFS.AbstractSphericalGrid,
    ...
    plan = NUFSHT.make_plan(θd, φd, lmax; tol = tol, T = FT, ntrans = B)
